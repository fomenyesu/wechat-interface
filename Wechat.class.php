...
class Wechat
{
	...
	*/
	...
	public function reply($msg=array(),$return = false)
	{
		...
		$this->logdebug($xmldata);
		if ($this->encrypt_type == 'aes') { //如果来源消息为加密方式
		    $pc = new Prpcrypt($this->encodingAesKey);
		    $array = $pc->encrypt($xmldata, $this->appid);
		    $ret = $array[0];
		    if ($ret != 0) {
		        $this->logdebug('encrypt err!');
		        return false;
		    }
		    $timestamp = time();
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($token&&$token!="") { //手动指定token，优先使用
		    $this->access_token=$token;
		    return $this->access_token;
		}
		$authnamearr = $this->getwebtoken();
		$authnamearr = $authnamearr['webtoken'];
		$authname = $authnamearr['access_token'];
		$nowtime = strtotime(date('Y-m-d H:i:s',time()));
		$authtime = $authnamearr['time'];
		if($nowtime-$authtime<7200){
				$this->access_token = $authname;
				return $this->access_token;
		}
		...
		if ($result)
		{
			...
			$json['expires_in'] = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
			$json['time']=strtotime(date('Y-m-d H:i:s',time()));
			$this->logwebtoken($json);
   ...
   "articles": [
	...
	public function uploadArticles($data){
		...
		$jsonteststr='{
		   "articles": [
				 {
								"thumb_media_id":"am8ZLuwzvUu_QtlvJ2Eeqd66Ast1Z6rnCk-ebkOpI9Yf7QHraJ_tIZOaF7DlWqIb",
								"author":"ttt",
					 "title":"Happy Day",
					 "content_source_url":"www.qq.com",
					 "content":"content",
		...
		$result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_UPLOADNEWS_URL.'access_token='.$this->access_token,$jsonteststr);
		print_r($result);
		if ($result)
		{
			$json = json_decode($result,true);
	...
	public function logwebtoken($hl){
		$ini = parse_ini_file('data/logwebtoken.ini', true);
		$handle = fopen(dirname(__FILE__).'/data/logwebtoken.ini','w');
		
		if($handle){
			fwrite($handle,'[webtoken]'.chr(13));
			foreach ($ini['webtoken'] as $key => $line){
				fwrite($handle,$key.' = "'.$hl[$key].'"'.chr(13));
			}
			fclose($handle);
		}
	}
